...
fn projects_flat_events_through_streaming_finalization_and_recovery() {
    ...
    let mut transcript = Transcript::new(&raw, &[], Some("model".into()), QueueState::default(), None).unwrap();
    ...
    transcript.apply(&transcript.project(&json!({"type":"live","entry":tool})).unwrap()).unwrap();
    ...
    let mut recovered = Transcript::new(&[user.clone(), model.clone(), saved.clone()], &[], Some("a".into()), QueueState::default(), Some(&transcript)).unwrap();
    ...
    let finalized = Transcript::new(&[user, model, saved, final_tool.clone()], &[], Some("result".into()), QueueState::default(), Some(&recovered)).unwrap();
...
fn pages_inside_messages_and_resolves_requests_outside_the_window() {
    ...
    let transcript = Transcript::new(&[user, assistant], &[], Some("a".into()), QueueState::default(), None).unwrap();
    ...
    assert_eq!(Transcript::new(&[orphan], &[], Some("a".into()), QueueState::default(), None).unwrap().snapshot(&[]).events.len(), 1);
    let cycle = [json!({"id":"a","parentId":"b"}), json!({"id":"b","parentId":"a"})];
    assert!(Transcript::new(&cycle, &[], Some("a".into()), QueueState::default(), None).is_err());
    let duplicate = [json!({"id":"a"}), json!({"id":"a"})];
    assert!(Transcript::new(&duplicate, &[], Some("a".into()), QueueState::default(), None).is_err());
}
...
fn preserves_identified_queue_controls_and_hides_binary_content() {
    ...
    let queue = QueueState::from_entry(&raw).unwrap();
    ...
    assert!(QueueState::from_entry(&duplicate).is_err());
}
...
fn recovery_restores_source_order_and_keeps_interrupted_work_between_its_neighbors() {
    ...
    let mut old = Transcript::new(&[user.clone(), closing.clone()], &[], Some("closing".into()), QueueState::default(), None).unwrap();
    ...
    old.apply(&old.project(&json!({"type":"live","entry":abandoned})).unwrap()).unwrap();
    old.interrupt();
    ...
    let mut restored = Transcript::new(&raw, std::slice::from_ref(&current), Some("next".into()), QueueState::default(), Some(&old)).unwrap();
    for _ in 0..3 {
        ...
        restored = Transcript::new(&raw, std::slice::from_ref(&current), Some("next".into()), QueueState::default(), Some(&restored)).unwrap();
    }
    let alternate = json!({"id":"alternate","type":"message","parentId":"u","message":{"role":"user","content":"Other branch"}});
    let switched = Transcript::new(&[user, alternate], &[], Some("alternate".into()), QueueState::default(), Some(&restored)).unwrap();
...
fn snapshot_places_live_content_after_its_parent_and_before_later_saved_messages() {
    ...
    let transcript = Transcript::new(&[user, notice], &[live], Some("notice".into()), QueueState::default(), None).unwrap();
    ...
    let old = Transcript::new(std::slice::from_ref(&model), &[lost], Some("model".into()), QueueState::default(), None).unwrap();
    ...
    let restored = Transcript::new(&[missing, model, next], &[], Some("next".into()), QueueState::default(), Some(&old)).unwrap();
