...
class LocalStoreTest {
    ...
    fun applies_flat_pages_deltas_and_queue_state_while_preserving_local_work() = runBlocking {
        ...
        try {
            ...
            val retainedRow = chat.rows.first()
            val reordered = listOf(distant.last().copy(order = 0), distant.first().copy(order = 1))
            assertTrue(store.applySnapshot(key, TranscriptCut("recovered", 0, reordered, waiting)))
            assertEquals(reordered, chat.rows.map { it.event })
            assertSame(retainedRow, chat.rows.last())
